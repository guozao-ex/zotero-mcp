...
| 可选 | Docker/WSL2 **或**一台小服务器——仅当你想把 Zotero 的 `translation-server` 自建为额外的标识符解析通道；不装它一切照常 |
...
- **`translation-server`（可选的 L3 通道）只有「通道层」验证**：10 条契约测试覆盖端点/令牌/超时/脱敏与优雅降级，
  并有一轮独立验收通过 —— 但**部署本身从未跑过**（无容器、无云端；开发机没有 Docker 也没有 WSL2）。
  至今**所有真机运行中该端点都不存在**，标识符解析走的是直连 Crossref / OpenLibrary / PubMed 的回退路径：
  也就是说「**不用它也能工作**」已被验证，「**用它也能工作**」没有。
